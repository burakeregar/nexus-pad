...
export async function registerForPushNotificationsAsync(): Promise<string | null> {
    ...
    try {
        ...
        if (projectId) {
            ...
            // In development without EAS, push tokens won't work but app should continue
            console.warn('[Notifications] No projectId found - push notifications disabled in development. Run eas build or configure EAS to enable.');
            return null;
        }

        console.log('[Notifications] Push token:', token);
    } catch (error: any) {
        // Gracefully handle missing projectId or other token errors
        if (error?.message?.includes('projectId')) {
            console.warn('[Notifications] Push notifications not available - no EAS projectId configured.');
        } else {
